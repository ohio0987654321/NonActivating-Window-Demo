...
#define REGISTRY_FILE_PATH "/tmp/window_modifier_registry.dat"
#define MAX_WINDOWS 1024
#define STALE_ENTRY_SECONDS 3600 // 1 hour
...
window_registry_t* registry_init(void) {
    ...
    sprintf(registry->file_path, "%s", REGISTRY_FILE_PATH);
    ...
    // Perform a non-blocking lock test
...
bool registry_is_window_modified(window_registry_t* registry, uint32_t window_id) {
    ...
    if (flock(registry->lock_fd, LOCK_SH) != 0) {
        printf("[Registry] Warning: Failed to acquire read lock\n");
        return false;
    }
...
bool registry_mark_window_modified(window_registry_t* registry, uint32_t window_id) {
    ...
    if (flock(registry->lock_fd, LOCK_EX) != 0) {
        printf("[Registry] Error: Failed to acquire write lock\n");
        return false;
    }
...
int registry_cleanup_stale(window_registry_t* registry) {
    ...
    // Acquire an exclusive lock for rewriting
    if (flock(registry->lock_fd, LOCK_EX) != 0) {
        printf("[Registry] Error: Failed to acquire exclusive lock for cleanup\n");
        return 0;
    }
    
    // Create a temporary file for the new registry
    char temp_file[520];
    sprintf(temp_file, "%s.tmp", registry->file_path);
    ...
    if (temp_fd < 0) {
        printf("[Registry] Error: Failed to create temp file for cleanup\n");
        flock(registry->lock_fd, LOCK_UN);
        return 0;
    }
    ...
    // First entry is header/initialization entry
    ...
    while ((bytes_read = read(registry->lock_fd, &entry, sizeof(registry_entry_t))) == sizeof(registry_entry_t)) {
        ...
        if (now - entry.timestamp > STALE_ENTRY_SECONDS) {
            // Process no longer exists or entry is too old
            removed_count++;
            continue;
        }
